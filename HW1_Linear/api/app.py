...
# Если запускать скрипт напрямую через консоль, то можно сразу проверить примеры работы API
# Если необходимо запустить сервис через сервер unicorn, то необходимо заккоментировать всё, что ниже, для корректного запуска в консоли
